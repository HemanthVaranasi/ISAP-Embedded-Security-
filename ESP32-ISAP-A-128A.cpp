...
void isap_a_128a_simd_core(const uint8_t* key, const uint8_t* nonce,
  ...
  state[2] = 0xa1a2a3a4a5a6a7a8;  // Variant-specific domain separation
  state[3] = 0;
  state[4] = 0;
...
void benchmark(size_t msg_len) {
  const int rounds = 1000;  // Adjust rounds for ESP32 performance
  uint8_t key[16] = {0};
  uint8_t nonce[16] = {0};
...
void setup() {
  ...
  Serial.println("\n*SIMD-Optimized ISAP-A-128A Benchmark (10 Rounds)*");
