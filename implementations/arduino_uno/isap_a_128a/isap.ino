// Arduino Uno compatible version of ISAP-A-128A benchmark code
#include <Arduino.h>
...
// ------------------ ISAP-A-128A CORE FUNCTION ----------------------
void isap_a_128a_simd_core(const uint8_t* key, const uint8_t* nonce,
...
void benchmark(size_t msg_len) {
  ...
  for (int i = 0; i < rounds; ++i)
    isap_a_128a_simd_core(key, nonce, plaintext, msg_len, ciphertext);
  unsigned long enc_end = micros();

  unsigned long dec_start = micros();
  for (int i = 0; i < rounds; ++i)
    isap_a_128a_simd_core(key, nonce, ciphertext, msg_len, decrypted);
  ...
  Serial.print("| "); Serial.print(msg_len); Serial.print(" B\t| ");
  Serial.print(enc_time, 2); Serial.print(" Âµs\t| ");
  Serial.print(dec_time, 2); Serial.print(" Âµs\t| ");
  Serial.print(mem_kb, 3); Serial.print(" KB\t| ");
  Serial.print(throughput, 2); Serial.println(" kbps |");
}
...
void setup() {
  ...
  Serial.println("\n*SIMD-Optimized ISAP-A-128A Benchmark (10 Rounds) – Arduino Uno*");
