...
// ------------ ISAP-K-128 ENCRYPT/DECRYPT MOCK ------------
void isap_k_128_simd_core(const uint8_t* key, const uint8_t* nonce,
  ...
  // Unique domain constant for ISAP-K-128
  state32[4] = 0x11121314;
  state32[5] = 0x15161718;
...
void benchmark(size_t msg_len) {
  ...
  for (int i = 0; i < rounds; ++i)
    isap_k_128_simd_core(key, nonce, plaintext, msg_len, ciphertext);
  int64_t enc_end = esp_timer_get_time();

  int64_t dec_start = esp_timer_get_time();
  for (int i = 0; i < rounds; ++i)
    isap_k_128_simd_core(key, nonce, ciphertext, msg_len, decrypted);
...
void setup() {
  ...
  Serial.println("\n*SIMD-Optimized ISAP-K-128 Benchmark (10 Rounds) – ESP32*");
